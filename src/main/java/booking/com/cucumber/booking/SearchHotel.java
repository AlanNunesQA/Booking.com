...
public class SearchHotel {
	...
	public SearchHotel() {
		// Chrome Driver for running tests, change the driver path according to where you saved it.
	...
	public void openViewResults() throws Throwable {
		...
		try {
			...
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath(Fields.VIEW_CLASS_DATE.getXPathByClass()))).click();
			// Simulation of user with double click for next month
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath(Fields.VIEW_CLASS_NEXT_MONTH.getXPathByClass())))
					.click();
			driver.findElement(By.xpath(Fields.VIEW_CLASS_NEXT_MONTH.getXPathByClass())).click();
			// Date selection
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath(Fields.VIEW_ID_SELECT_DATE.getXPathByData()))).click();
			// Submit form
			driver.findElement(By.xpath(Fields.VIEW_CLASS_BUTTON_SUBMIT.getXPathByClass())).click();
	...
	public void selectFilter(String filter) throws Throwable {
		...
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath(Fields.VIEW_CLASS_LABEL_FILTERS.getXPathByClassAndFilter(filter)))).click();
			// Wait for filter application
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath(Fields.VIEW_APPLIED_FILTER.getXPathByText())))
					.isEnabled();
		} catch (NoSuchElementException | TimeoutException e) {
	...
	public void checkHotelName(String hotelname) throws Throwable {
		...
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath(Fields.VIEW_CLASS_NAME_HOTEL_FILTERS.getXPathByClassAndFilter(hotelname))));
